...
const HeroDelivery = () => {
  return (
    <section className='w-full flex justify-center md:py-20'>
          ...
          {allServices.map((item)=> (
            <div key={item.id} className='flex flex-col items-center gap-4'>
                ...
                <div className='text-center space-y-1'>
                    <h2 className='text-[16px] font-bold text-black/90 '>{item.title}</h2>
                <p className='text-[12px] font-semibold text-black/90'>{item.text}</p>
                </div>
            </div>
          ))}
        </div>
