...
// for auto focus
  ...
  return (
    <div>
      {searchItem && (
        <div
          className=" flex flex-col items-center justify-start pt-28 fixed inset-0 bg-black/25 backdrop-blur-sm top-[112px] left-0 bottom-0 right-0 z-20"
        >
              ...
              {data && data?.length > 0 ? (
                data?.map((item: any)=> (
                  <li key={item.id} onClick={handleModal}>
                    <Link className="block px-4 py-2 bg-white hover:bg-gray-100 rounded" href={`/shop/${item.id}`}>{item.title}</Link>
                    </li>
                ))
         ...
         <div onClick={handleModal}  className="absolute right-3 top-3 md:right-8 md:top-5 cursor-pointer">
